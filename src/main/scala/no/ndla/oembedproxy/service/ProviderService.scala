...
import no.ndla.oembedproxy.service.OEmbedConverterService.{addYoutubeTimestampIfdefinedInRequest, removeQueryString, removeQueryStringAndFragment, cleanYoutubeRequestUrl}
...
trait ProviderService {
  ...
  class ProviderService extends LazyLogging {
    ...
    val YoutuProvider = OEmbedProvider("YouTube", "http://youtu.be", List(YoutubeEndpoint), cleanYoutubeRequestUrl, addYoutubeTimestampIfdefinedInRequest)
    val YoutubeProvider = OEmbedProvider("YouTube", "http://www.youtube.com", List(YoutubeEndpoint), cleanYoutubeRequestUrl, addYoutubeTimestampIfdefinedInRequest)
    ...
    val NdlaApiProvider = OEmbedProvider("NDLA Api", OEmbedProxyProperties.NdlaApiOembedProvider, List(NdlaApiEndpoint), removeQueryString)
    ...
    def _loadProviders(): List[OEmbedProvider] = {
      NdlaApiProvider :: TedProvider :: H5PProvider :: HttpNdlaProvider :: HttpsNdlaProvider :: YoutubeProvider :: YoutuProvider :: GoOpenProvider :: IssuuProvider :: loadProvidersFromRequest(Http(OEmbedProxyProperties.JSonProviderUrl))
    }
