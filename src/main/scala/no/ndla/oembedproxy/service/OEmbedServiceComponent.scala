...
trait OEmbedServiceComponent extends LazyLogging {
  ...
  class OEmbedService(optionalProviders: Option[List[OEmbedProvider]] = None) {
    ...
    private def fetchOembedFromProvider(provider: OEmbedProvider, url: String, maxWidth: Option[String], maxHeight: Option[String]): Try[OEmbed] = {
      ndlaClient.fetch[OEmbed](Http(provider.requestUrl(url, maxWidth, maxHeight)).option(HttpOptions.followRedirects(true)))
    }
    ...
    def get(url: String, maxWidth: Option[String], maxHeight: Option[String]): Try[OEmbed] = {
      getProvider(url) match {
        ...
        case Some(provider) => fetchOembedFromProvider(provider, url, maxWidth, maxHeight).map(provider.postProcessor(url, _))
      }
    }
